...
- **current full-closure score: 5/53 (9%) C**, all fully production-refined
  and integrated.
...
| S08 | Barrier: `src/sync/barrier.rs` | new/wait, reusable cohorts and leader result | `sync` | **L / R / I**; `barrier_{protocol,refinement}.rs` | Mutex, watch storage/wake, liveness | The watch dependency is now closed by S03. Re-audit the composition for current closure; the whole-word-stalled follower remains the declared finite-counter liveness boundary. |
...
1. S05 mpsc index/block/reclamation and last-close completion refinement.
