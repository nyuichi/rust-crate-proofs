...
| S04 | broadcast: `src/sync/broadcast.rs` | channel, send/recv/try_recv, lag, subscribe, weak/endpoints | `sync` | **L / R(partial) / I**; `broadcast_{protocol,completion,refinement}.rs`, `BROADCAST-{MUTATION,WRAP}-AUDIT.md`; exact wrapping distance/advance, subscribe snapshot, send reservation, empty/ready/lag transitions, retained-slot drain count, mask bounds, initial slot tag, publish/overwrite generation tag, and wrap-crossing witnesses are body-proved; focused production and loom regressions are integrated | atomics, Mutex, raw waiter links, Waker, Clone/Drop execution | The finite observation window is a conditional proof precondition, not a selected production policy. Full-cycle ABA remains open. Production `Receiver::{len,is_empty}` and `<`-bounded Drop drain do not refine the proved wrapping/bounded operations and require a reviewed production change. The sequential drop model does not cover sends concurrent with the unlocked production drain. Generic mask-generation equivalence, physical slot/`rem` coupling, waiter/slot lock orchestration, weak/endpoints, public trait/error surfaces, and 32-bit conversion behavior remain before C/R. |
