...
- **current full-closure score: 3/53 (6%) C**, all fully production-refined
  and integrated.
...
| S02 | oneshot: `src/sync/oneshot.rs`, `oneshot_value.rs` | channel, Sender/Receiver, poll/close/drop | `sync`; internal under `rt` and Windows `process` | **L / R(partial) / I**; `oneshot_{state,atomic,value,poll,closed,drop}.rs`, `ONESHOT-MUTATION-AUDIT.md` | atomics, UnsafeCell, Pin/Poll/Waker, user Drop execution | Refine the full production orchestration and physical payload slot rather than only its operation adapter; then upgrade to C/R. |
| S03 | watch: `src/sync/watch.rs` | channel, send/update, borrow/changed/wait_for, endpoint lifecycle | `sync` | **L / P / I** under the earlier channel scope; `watch_{protocol,completion}.rs` | atomics, RwLock, Waker, Clone/Drop execution | Treat predicate/update-closure results as specification inputs; prove their exact value/generation effects plus production BigNotify/fanout and wrapping update/poll paths to reach C/R. |
| S04 | broadcast: `src/sync/broadcast.rs` | channel, send/recv/try_recv, lag, subscribe, weak/endpoints | `sync` | **L / P / I** under the earlier channel scope; `broadcast_{protocol,completion}.rs` | atomics, Mutex, raw waiter links, Waker, Clone/Drop execution | Refine production `u64` tail wrap, mask/slot generation, and waiter/slot orchestration. |
| S05 | mpsc: `src/sync/mpsc/{chan,list,block,bounded,unbounded}.rs` | bounded/unbounded send/reserve/recv/close, permits, weak/endpoints | `sync`; timeout forms also `time` | **L / P / I** under the earlier channel scope; `mpsc_{protocol,completion}.rs` | semaphore/AtomicWaker adapters, atomics, raw allocation/pointers, Drop execution, liveness | Refine index wrap, arbitrary block-chain reuse/reclamation, weak-count representation, and production poll orchestration. |
...
| S07 | Notify: `src/sync/notify.rs` | notify_one/last/waiters, Notified/OwnedNotified enable/poll/drop | `sync`; internal under `rt`, `signal`, `process` | **C / R / I**; `notified_core.rs`, `notify_{protocol,refinement}.rs` | atomics, Mutex, raw links, Pin/Poll/Waker | Closed under frozen scope. |
...
| T01 | cooperative yield/budget: `src/task/{yield_now,coop/*}` | yield_now, poll_proceed/consume_budget/unconstrained | `rt`; `test-util` controls | **U** | task-local/context access, Waker/liveness | Prove yield register/recheck plus budget conservation, forced Pending registration, restoration, and unconstrained nesting. |
...
3. S05 mpsc index/block/weak-count/reclamation refinement.
