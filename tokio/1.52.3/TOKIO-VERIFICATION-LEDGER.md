...
| S04 | broadcast: `src/sync/broadcast.rs` | channel, send/recv/try_recv, lag, subscribe, weak/endpoints | `sync` | **L / R(partial) / I**; `broadcast_{protocol,completion,refinement}.rs`, `BROADCAST-{MUTATION,WRAP}-AUDIT.md`; selected all-build terminal exhaustion, no-Receiver error priority, mutation-free terminal gate, monotonic reservation through ticket `MAX-1`, exact/saturating len and direct emptiness, a capacity-two terminal queried-slot witness, positive lag, retained drain, and post-snapshot concurrent-send exclusion are body-proved and covered by focused production/loom regressions | atomics, Mutex, raw waiter links, Waker, Clone/Drop execution | Full-cycle ABA, debug/release rollover, 32-bit len truncation, wrapping Drop, and post-snapshot release are closed. Generic mask-generation equivalence, direct physical slot/`rem` coupling, waiter/slot lock orchestration, weak endpoint atomics, and public trait/error surfaces remain before C/R. Terminal exhaustion is not channel close: an empty terminal Receiver may remain Pending until Sender drop. |
