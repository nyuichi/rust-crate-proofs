...
| R02 | task core/state: `src/runtime/task/{state,core,raw,harness,waker}.rs` | downstream basis of spawned tasks | `rt` | **P / R(partial) / I**; R02-1 via `verification/src/task_state_refinement.rs`, `RUNTIME-TASK-CLOSURE-CHECKLIST.md`; exact six-bit packed state, initial three references, lifecycle, notification, cancellation, join flags, and reference transitions are body-proved and directly tested | atomics, raw allocation/vtable, Pin/Poll/Waker, panic/Drop; finite simultaneous references | R02-1 closed. Connect spawn/poll/complete, abort/join ownership, then raw deallocation and unwind before claiming C. |
