...
| S05 | mpsc: `src/sync/mpsc/{chan,list,block,bounded,unbounded}.rs` | bounded/unbounded send/reserve/recv/close, permits, weak/endpoints | `sync`; timeout forms also `time` | **L / R(partial) / I**; `mpsc_{protocol,completion,refinement}.rs`, `MPSC-{MUTATION,WRAP}-AUDIT.md`; exact `recv`/`recv_many` first-pop/register/recheck priorities, latest-Waker retention, reserved-permit send/cancel continuations, bounded single/batch permit restoration, logical-oracle observation-order batches, unbounded encoded-count arithmetic, limit-zero behavior, bounded/unbounded public-surface tests | S06 semaphore and S09 AtomicWaker contracts; atomics, UnsafeCell, Pin/Poll/Context/Waker, raw allocation/pointers, Drop execution, liveness | `QueueRead` remains a logical oracle not yet connected to the raw list/slot chain. Gate preservation covers only Waker/returned-permit/progress bookkeeping, not queue/buffer/capacity snapshots. Fix/review final-index `grow`/`has_value` overflow, then refine index generations, block geometry/reuse/reclamation, weak-count representation, `try_recv` Busy park loop, trace/coop execution, and recv-many allocation/panic-guard/drop paths. |
