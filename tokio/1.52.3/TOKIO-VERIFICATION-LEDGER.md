...
| S02 | oneshot: `src/sync/oneshot.rs`, `oneshot_value.rs` | channel, Sender/Receiver, poll/close/drop | `sync`; internal under `rt` and Windows `process` | **C / R / I**; `oneshot_{state,atomic,value,poll,closed,drop,refinement,shared_refinement,surface}.rs`, `vstd_ext/shared_pcell.rs`, `ONESHOT-{MUTATION-AUDIT,CLOSURE-CHECKLIST}.md`; exact four-bit/CAS and shared-cell permission handoff, Waker transition-in-progress ownership, register/recheck races, endpoint/local-Arc cleanup, trace/coop/Future terminal surface, blocking paths, errors/traits and cfg gates are proved or tested | raw atomic/UnsafeCell semantics, Arc, Pin/Poll/Context/Waker, arbitrary user Drop/formatting execution, scheduler/parking liveness | Closed above the frozen foundation. `SharedPCell` is body-proved from vstd `PCell`, not a new trusted adapter. Taskdump callback mechanics remain owned by R08; S02 proves their pre-state ordering and preservation. |
...
| F01 | future combinators: `src/future/{maybe_done,try_join}.rs`, `src/macros/try_join.rs` | `try_join!` orchestration and internal MaybeDone | `macros` for macro; core future internals | **C / R / I**; `verification/src/future_try_join_refinement.rs`, `FUTURE-TRY-JOIN-CLOSURE-CHECKLIST.md`; Future/Done/Gone ownership, output mutation/take, Gone repoll panic, private try_join3 ordered success/early-error/cancellation, normal rotation, biased order, recursive macro arity, and output Drop are proved or tested | Pin/Poll/Context/Waker mechanics, arbitrary inner Future and output/error Drop execution | Closed above the frozen foundation. The canonical three-branch rotator is body-proved; recursive expansion through eight branches and existing fairness/size cases connect the public macro. |
...
| R02 | task core/state: `src/runtime/task/{state,core,raw,harness,waker}.rs` | downstream basis of spawned tasks | `rt` | **P / R(partial) / I**; R02-1/2 via `verification/src/{task_state,task_harness}_refinement.rs`, `RUNTIME-TASK-CLOSURE-CHECKLIST.md`; exact packed state plus spawn, notification-reference transfer, pending, self-wake resubmission, Ready output storage, and completion are body-proved and tested on both schedulers | atomics, raw allocation/vtable, Pin/Poll/Waker, panic/Drop; finite simultaneous references | Normal orchestration closed. Connect abort/join ownership, then raw deallocation and unwind before claiming C. |
