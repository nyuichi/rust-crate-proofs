...
#[cfg(not(creusot))]
use core::fmt;
#[cfg(all(any(feature = "std", test), not(creusot)))]
use std::error;
...
use creusot_std::prelude::{
    ensures, invariant, logic, pearlite, requires, snapshot, trusted, DeepModel, Int, Seq,
};

/// Mathematical length of a base64 encoding.
#[logic(open)]
pub fn encoded_len_model(bytes_len: Int, padding: bool) -> Int {
    pearlite! {
        4 * (bytes_len / 3)
            + if bytes_len % 3 == 0 {
                0
...
#[cfg(all(any(feature = "alloc", test), not(creusot)))]
...
// Creusot boundary: generic AsRef/Engine dispatch is not modeled.
// TODO: remove after the Engine slice contract and AsRef bridge are available.
#[trusted]
...
// Creusot boundary: orchestration depends on the trusted codec body below.
// TODO: remove after internal_encode has an RFC-level contract and proved body.
#[trusted]
...
#[ensures(match result {
    Some(len) => len@ == encoded_len_model(bytes_len@, padding),
    None => encoded_len_model(bytes_len@, padding) > usize::MAX@,
})]
...
/// `output` is the slice where padding should be written, of length at least 3.
///
/// Returns the number of padding bytes written.
#[requires(output@.len() >= 3)]
#[ensures(result@ == (4 - unpadded_output_len@ % 4) % 4)]
#[ensures(result@ > 0 ==> output@[0]@ == PAD_BYTE@)]
#[ensures(result@ >= 2 ==> output@[1]@ == PAD_BYTE@)]
#[ensures(result@ == 3 ==> output@[2]@ == PAD_BYTE@)]
// Creusot boundary: the reviewed contract is exact for count and written bytes.
// TODO: prove the finite 0/1/2/3-byte write directly.
#[trusted]
