...
use creusot_std::prelude::{ensures, requires, trusted, DeepModel};
...
#[cfg_attr(not(creusot), derive(Clone, Debug, Eq, PartialEq))]
pub struct Alphabet {
    #[cfg(not(creusot))]
    pub(crate) symbols: [u8; ALPHABET_SIZE],
    #[cfg(creusot)]
    pub symbols: [u8; ALPHABET_SIZE],
}

impl Alphabet {
    /// Performs no checks so that it can be const.
    /// Used only for known-valid strings.
    // Creusot boundary: reviewed contract exposes the exact 64-byte copy.
    // TODO: remove when const str byte access and its loop can be proved directly.
    #[trusted]
    #[requires(alphabet@.to_bytes().len() == ALPHABET_SIZE@)]
    #[ensures(result.symbols@ == alphabet@.to_bytes())]
    ...
    // Creusot boundary: nested uniqueness validation is not yet specified.
    // TODO: add exact error-precedence contracts, then prove both loops.
    #[trusted]
    ...
    // Creusot boundary: from_utf8/unwrap lacks the required pinned contract.
    // TODO: remove after printable-byte invariants and from_utf8 are connected.
    #[trusted]
...
impl convert::TryFrom<&str> for Alphabet {
    ...
    // Creusot boundary: delegates to the trusted validation constructor.
    // TODO: remove together with Alphabet::new.
    #[trusted]
