...
#[cfg_attr(not(creusot), derive(Debug, Clone))]
pub struct GeneralPurpose {
    #[cfg(not(creusot))]
    encode_table: [u8; 64],
    #[cfg(creusot)]
    pub encode_table: [u8; 64],
    #[cfg(not(creusot))]
    decode_table: [u8; 256],
    #[cfg(creusot)]
    pub decode_table: [u8; 256],
    #[cfg(not(creusot))]
    config: GeneralPurposeConfig,
    #[cfg(creusot)]
    pub config: GeneralPurposeConfig,
}
...
impl GeneralPurpose {
    ...
    // Creusot boundary: table construction is separated from config proofs.
    // TODO: remove after encode_table/decode_table have proved alphabet relations.
    #[trusted]
...
impl super::Engine for GeneralPurpose {
    ...
    // Creusot boundary: optimized 24-byte loop has no RFC-level model yet.
    // TODO: prove a 3-byte step, remainder, then the fast-loop refinement.
    #[trusted]
    ...
    // Creusot boundary: delegates to trusted decode chunks and suffix validation.
    // TODO: remove bottom-up after those component bodies are proved.
    #[trusted]
    fn internal_decode(
        &self,
        input: &[u8],
...
// Creusot boundary: TODO prove this table copies all 64 alphabet symbols.
#[trusted]
...
// Creusot boundary: TODO prove valid symbols map back to their six-bit indices.
#[trusted]
...
#[inline]
// Creusot boundary: TODO use a repository-owned from_be_bytes slice contract.
#[trusted]
...
impl GeneralPurposeConfig {
    ...
    #[ensures(result.encode_padding)]
    #[ensures(!result.decode_allow_trailing_bits)]
    #[ensures(result.decode_padding_mode.deep_model() ==
        DecodePaddingMode::RequireCanonical.deep_model())]
    ...
    #[ensures(result.encode_padding == padding)]
    #[ensures(result.decode_allow_trailing_bits == self.decode_allow_trailing_bits)]
    #[ensures(result.decode_padding_mode.deep_model() == self.decode_padding_mode.deep_model())]
    ...
    #[ensures(result.encode_padding == self.encode_padding)]
    #[ensures(result.decode_allow_trailing_bits == allow)]
    #[ensures(result.decode_padding_mode.deep_model() == self.decode_padding_mode.deep_model())]
    ...
    #[ensures(result.encode_padding == self.encode_padding)]
    #[ensures(result.decode_allow_trailing_bits == self.decode_allow_trailing_bits)]
    #[ensures(result.decode_padding_mode.deep_model() == mode.deep_model())]
...
impl Config for GeneralPurposeConfig {
    #[ensures(result == self.encode_padding)]
