...
use creusot_std::prelude::{ensures, logic, pearlite, trusted, Int};
#[cfg(all(any(feature = "std", test), not(creusot)))]
use std::error;

/// Conservative decoded buffer length, rounded up to a complete three-byte block.
#[logic(open)]
pub fn decoded_len_estimate_model(encoded_len: Int) -> Int {
    pearlite! {
        3 * (encoded_len / 4 + if encoded_len % 4 > 0 { 1 } else { 0 })
    }
}
...
impl From<DecodeError> for DecodeSliceError {
    // Creusot boundary: adapter only; no codec fact depends on this conversion.
    // TODO: remove when enum conversion refinement is stable in the pinned toolchain.
    #[trusted]
...
/// ```
#[ensures(result@ == decoded_len_estimate_model(encoded_len@))]
