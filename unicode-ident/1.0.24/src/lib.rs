...
use creusot_std::__stubs::dead;
#[cfg(creusot)]
#[allow(unused_imports)]
use creusot_std::prelude::{ensures, logic, pearlite, requires, trusted, Int, View};
...
pub fn trie_start_model(index: Int) -> Int {
    let _ = index;
    dead
}
...
#[logic(opaque)]
#[allow(unused_variables)]
pub fn trie_continue_model(index: Int) -> Int {
    let _ = index;
    dead
}
...
#[logic(opaque)]
#[allow(unused_variables)]
pub fn leaf_model(index: Int) -> Int {
    let _ = index;
    dead
}
...
#[cfg(creusot)]
#[logic(open)]
pub fn bit_divisor(bit: Int) -> Int {
    if bit == 0 {
        1
    } else if bit == 1 {
        2
...
#[cfg(creusot)]
#[logic(open)]
pub fn table_bit(byte: Int, bit: Int) -> bool {
    pearlite! { (byte / bit_divisor(bit)) % 2 == 1 }
}

#[cfg(creusot)]
#[logic(open)]
pub fn xid_start_model(ch: char) -> bool {
    pearlite! {
        if ch@ < 128 {
            (65 <= ch@ && ch@ <= 90) || (97 <= ch@ && ch@ <= 122)
        } else {
            let chunk = trie_start_model(ch@ / 512);
            let offset = chunk * 32 + (ch@ / 8) % 64;
            table_bit(leaf_model(offset), ch@ % 8)
        }
    }
}
...
#[cfg(creusot)]
#[logic(open)]
pub fn xid_continue_model(ch: char) -> bool {
    pearlite! {
        if ch@ < 128 {
            (48 <= ch@ && ch@ <= 57)
                || (65 <= ch@ && ch@ <= 90)
            ...
            let chunk = trie_continue_model(ch@ / 512);
            let offset = chunk * 32 + (ch@ / 8) % 64;
            table_bit(leaf_model(offset), ch@ % 8)
        }
    }
...
// Trusted generated-data boundary. These accessors represent the bytes in the
// published Unicode 17.0.0 tables. Their contracts are the removal condition:
// expose immutable statics to Creusot and prove each accessor against them.
#[cfg(creusot)]
#[trusted]
#[ensures(result@ == ch@)]
fn char_codepoint(ch: char) -> u32 {
    ch as u32
}

#[cfg(creusot)]
#[trusted]
#[ensures(result@ == trie_start_model(index@))]
#[ensures(result@ <= 242)]
fn trie_start_at(index: usize) -> u8 {
    let _ = index;
    0
}
...
#[cfg(creusot)]
#[trusted]
#[ensures(result@ == trie_continue_model(index@))]
#[ensures(result@ <= 242)]
fn trie_continue_at(index: usize) -> u8 {
    let _ = index;
    0
}
...
#[trusted]
#[requires(index@ < 7808)]
#[ensures(result@ == leaf_model(index@))]
fn leaf_at(index: usize) -> u8 {
    let _ = index;
    0
}

#[cfg(creusot)]
#[ensures(result == table_bit(byte@, bit@))]
fn byte_bit(byte: u8, bit: u32) -> bool {
    match bit {
        0 => byte % 2 == 1,
        1 => (byte / 2) % 2 == 1,
        2 => (byte / 4) % 2 == 1,
...
#[cfg(creusot)]
#[requires(ch@ >= 128)]
#[ensures(result == xid_start_model(ch))]
fn is_xid_start_nonascii(ch: char) -> bool {
    let codepoint = char_codepoint(ch) as usize;
    let chunk = trie_start_at(codepoint / 8 / 64);
    let offset = chunk as usize * 32 + codepoint / 8 % 64;
    byte_bit(leaf_at(offset), char_codepoint(ch) % 8)
}

#[cfg(creusot)]
#[requires(ch@ >= 128)]
#[ensures(result == xid_continue_model(ch))]
fn is_xid_continue_nonascii(ch: char) -> bool {
    let codepoint = char_codepoint(ch) as usize;
    let chunk = trie_continue_at(codepoint / 8 / 64);
    let offset = chunk as usize * 32 + codepoint / 8 % 64;
    byte_bit(leaf_at(offset), char_codepoint(ch) % 8)
}

/// Whether the character has the Unicode property XID\_Start.
#[cfg_attr(creusot, ensures(result == xid_start_model(ch)))]
pub fn is_xid_start(ch: char) -> bool {
    #[cfg(creusot)]
    {
        let codepoint = char_codepoint(ch);
        return if codepoint < 128 {
            (b'A' as u32 <= codepoint && codepoint <= b'Z' as u32)
                || (b'a' as u32 <= codepoint && codepoint <= b'z' as u32)
        } else {
            is_xid_start_nonascii(ch)
        };
    }
    #[cfg(not(creusot))]
    {
...
#[cfg_attr(creusot, ensures(result == xid_continue_model(ch)))]
pub fn is_xid_continue(ch: char) -> bool {
    #[cfg(creusot)]
    {
        let codepoint = char_codepoint(ch);
        return if codepoint < 128 {
            (b'0' as u32 <= codepoint && codepoint <= b'9' as u32)
                || (b'A' as u32 <= codepoint && codepoint <= b'Z' as u32)
                || codepoint == b'_' as u32
                || (b'a' as u32 <= codepoint && codepoint <= b'z' as u32)
        } else {
            is_xid_continue_nonascii(ch)
        };
    }
    #[cfg(not(creusot))]
    {
