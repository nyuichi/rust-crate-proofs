...
use creusot_std::prelude::{ensures, invariant, Int, View};
...
pub fn first_non_ascii_byte(slice: &[u8]) -> usize {
    // The published implementation below uses raw pointers and SSE2. Creusot
    // proves the equivalent scalar search against the shared exact contract.
    #[cfg(creusot)]
    {
        let mut i = 0;
    ...
    #[cfg(all(not(creusot), any(miri, not(target_arch = "x86_64"))))]
    {
        first_non_ascii_byte_fallback(slice)
    }

    #[cfg(all(not(creusot), not(miri), target_arch = "x86_64"))]
    {
