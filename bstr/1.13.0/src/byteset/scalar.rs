...
#[ensures(match result {
    Some(i) => i@ < haystack@.len()
        && haystack@[i@] != n1
        && (forall<j: Int> 0 <= j && j < i@ ==> haystack@[j] == n1),
    None => forall<j: Int> 0 <= j && j < haystack@.len() ==> haystack@[j] == n1,
})]
pub fn inv_memchr(n1: u8, haystack: &[u8]) -> Option<usize> {
    #[cfg(creusot)]
    {
        ...
        #[invariant(forall<j: Int> 0 <= j && j < i@ ==> haystack@[j] == n1)]
...
#[ensures(match result {
    Some(i) => i@ < haystack@.len()
        && haystack@[i@] != n1
        && (forall<j: Int> i@ < j && j < haystack@.len() ==> haystack@[j] == n1),
    None => forall<j: Int> 0 <= j && j < haystack@.len() ==> haystack@[j] == n1,
})]
pub fn inv_memrchr(n1: u8, haystack: &[u8]) -> Option<usize> {
    #[cfg(creusot)]
    {
        ...
        #[invariant(i@ <= haystack@.len())]
        #[invariant(forall<j: Int> i@ <= j && j < haystack@.len() ==> haystack@[j] == n1)]
        while i > 0 {
            i -= 1;
            if haystack[i] != n1 {
                return Some(i);
