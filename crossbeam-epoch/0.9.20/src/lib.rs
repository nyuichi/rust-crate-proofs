...
#[cfg(all(crossbeam_loom, not(creusot)))]
...
#[cfg(all(target_has_atomic = "ptr", not(creusot)))]
...
// Creusot cannot currently translate the runtime implementation's atomic raw
// pointers, thread-local participant registry, or deferred destructors.  The
// verification projection proves the exclusive epoch state machine while
// ordinary builds above retain the complete upstream implementation.
#[cfg(all(feature = "alloc", target_has_atomic = "ptr", creusot))]
mod verification;
#[cfg(all(feature = "alloc", target_has_atomic = "ptr", creusot))]
pub use crate::verification::Collector;
