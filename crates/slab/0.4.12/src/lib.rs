...
/// The logical view of a slab is its number of occupied entries.
impl<T> View for Slab<T> {
    type ViewTy = Int;
...
impl<T> Invariant for Slab<T> {
    #[logic(prophetic)]
    fn invariant(self) -> bool {
        pearlite! { self@ <= self.entries@.len() && self.next@ <= self.entries@.len() }
    }
}
...
// The occupied sequence is hidden behind `Entry<T>` and the standard Vec
// iterator adapters. Keep the iterator protocols usable without claiming an
// element-order model until that representation is modeled directly.
...
weak_iterator_spec!([T], IntoIter<T>);
weak_iterator_spec!(['a, T], Iter<'a, T>);
weak_iterator_spec!(['a, T], IterMut<'a, T>);
weak_iterator_spec!(['a, T], Drain<'a, T>);
...
impl<T> Slab<T> {
    ...
    /// ```
    #[trusted]
    #[ensures(match result { Some(_) => (^self)@ + 1 == self@, None => (^self)@ == self@ })]
