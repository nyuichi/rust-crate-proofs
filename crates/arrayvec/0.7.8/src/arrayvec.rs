...
impl<T, const CAP: usize> View for ArrayVec<T, CAP> {
    type ViewTy = Int;
    ...
    ($method_name:expr, $index:expr, $len:expr) => {
        {
            ...
            panic!(concat!("ArrayVec::", $method_name, ": index {} is out of bounds in vector of length {}"),
                   $index, $len);
            #[cfg(creusot)]
            panic!("ArrayVec index out of bounds")
        }
    }
}
...
impl<T, const CAP: usize> ArrayVec<T, CAP> {
    ...
    pub fn insert(&mut self, index: usize, element: T) {
        ...
        if self.try_insert(index, element).is_err() {
            panic!("ArrayVec insertion failed")
        }
    }
...
#[cfg(all(feature = "zeroize", not(creusot)))]
...
impl<'a, T: 'a, const CAP: usize> IteratorSpec for Drain<'a, T, CAP> {
    #[logic(open, prophetic)]
    fn produces(self, _visited: Seq<Self::Item>, _o: Self) -> bool { true }
...
impl<'a, T: 'a, const CAP: usize> DoubleEndedIteratorSpec for Drain<'a, T, CAP> {
    #[logic(open, prophetic)]
    fn produces_back(self, _visited: Seq<Self::Item>, _o: Self) -> bool { true }
...
#[cfg(not(creusot))]
impl<T, const CAP: usize> FromIteratorSpec<T> for ArrayVec<T, CAP> {
    #[logic(open)]
    fn from_iter_post(_prod: Seq<T>, _res: Self) -> bool { true }
}
...
#[cfg(all(feature = "borsh", not(creusot)))]
