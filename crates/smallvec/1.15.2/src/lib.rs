...
use creusot_std::prelude::{
    check, ensures, logic, pearlite, requires, trusted, DeepModel, Int, Invariant, View,
};
...
unsafe fn deallocate<T>(ptr: NonNull<T>, capacity: usize) {
    ...
    #[cfg(creusot)]
    let layout = match layout_array::<T>(capacity) {
        Ok(layout) => layout,
        Err(_) => panic!("invalid allocation layout"),
    };
...
#[cfg(not(feature = "union"))]
impl<A: Array> View for SmallVec<A> {
    type ViewTy = Int;

    /// Logical number of initialized elements. The element sequence itself is
    /// intentionally opaque while the raw-storage proof boundary remains.
    #[logic]
    fn view(self) -> Int {
        match self.data {
            SmallVecData::Inline(_) => self.capacity.deep_model(),
            SmallVecData::Heap { len, .. } => len.deep_model(),
        }
    }
}

#[cfg(not(feature = "union"))]
impl<A: Array> Invariant for SmallVec<A> {
...
impl<A: Array> SmallVec<A> {
    ...
    pub fn len(&self) -> usize {
        ...
        {
            match &self.data {
                SmallVecData::Inline(_) => self.capacity,
                SmallVecData::Heap { len, .. } => *len,
            }
        }
    }
    ...
    #[trusted]
    #[ensures((^self)@ == if self@ <= len@ { self@ } else { len@ })]
    ...
    /// Panics if `index > len`.
    #[trusted]
    #[requires(index@ <= self@ && self@ < usize::MAX@)]
    #[ensures((^self)@ == self@ + 1)]
    ...
    A::Item: Copy,
{
    ...
    #[inline]
    #[trusted]
    #[requires(index@ <= self@ && self@ + slice@.len() <= usize::MAX@)]
    #[ensures((^self)@ == self@ + slice@.len())]
    ...
    #[inline]
    #[trusted]
    #[requires(self@ + slice@.len() <= usize::MAX@)]
    #[ensures((^self)@ == self@ + slice@.len())]
