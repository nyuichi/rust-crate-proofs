...
use crate::FxHasher;
use creusot_std::prelude::{ensures, logic, trusted, Invariant, View};
...
impl View for FxRandomState {
    type ViewTy = usize;

    /// The cached seed used to construct hashers from this state.
    #[logic]
...
impl Invariant for FxRandomState {
    #[logic(open)]
...
impl FxRandomState {
    ...
    #[trusted]
    // TODO: remove this boundary when Creusot models thread-local state and
    // rand's `Rng::random_range` protocol.
    pub fn new() -> FxRandomState {
        // Creusot cannot translate the thread-local/RNG protocol below. The
        // verification build keeps it as an explicit trusted boundary; normal
        // builds continue to execute the upstream implementation unchanged.
        #[cfg(creusot)]
        return FxRandomState { seed: 0 };
