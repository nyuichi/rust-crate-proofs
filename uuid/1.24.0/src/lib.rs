...
mod non_nil;
#[cfg(creusot)]
#[path = "verify/non_nil.rs"]
mod non_nil;
#[cfg_attr(creusot, creusot::no_translate)]
#[cfg(not(creusot))]
mod parser;
#[cfg(creusot)]
#[path = "verify/parser.rs"]
mod parser;
...
#[cfg_attr(not(creusot), derive(Clone, Copy, Debug, PartialEq))]
...
impl View for Uuid {
    ...
    /// The exact 16 octets stored by this UUID.
    #[logic]
...
impl Uuid {
    ...
    ///
    /// * [Variant Field in RFC 9562](https://www.ietf.org/rfc/rfc9562.html#section-4.1)
    #[ensures(result.deep_model() == if (self@[8] & 0x80u8) == 0u8 {
        0u8
    } else if (self@[8] & 0xc0u8) == 0x80u8 {
        1u8
    } else if (self@[8] & 0xe0u8) == 0xc0u8 {
        2u8
    } else {
        3u8
    })]
    ...
    ///
    /// * [Version Field in RFC 9562](https://www.ietf.org/rfc/rfc9562.html#section-4.2)
    #[ensures(result == (self@[6] >> 4u8) as usize)]
    ...
    /// # }
    /// ```
    #[ensures(result@ == self@.set(0, self@[3]).set(1, self@[2]).set(2, self@[1]).set(3, self@[0])
        .set(4, self@[5]).set(5, self@[4]).set(6, self@[7]).set(7, self@[6]))]
