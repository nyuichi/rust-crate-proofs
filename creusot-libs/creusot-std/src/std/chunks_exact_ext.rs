...
fn chunks_exact_ext_view<'a, T>(slice: &'a [T], chunk_size: usize) -> (Seq<Seq<T>>, Seq<T>, Int) {
    pearlite! {
...
#[logic(open)]
pub fn chunks_exact_partition<T>(slice: Seq<T>, remainder: Seq<T>) -> bool {
    pearlite! {
        slice == slice.subsequence(0, slice.len() - remainder.len()).concat(remainder)
    }
}

#[logic(open)]
pub fn chunks_exact_remainder_facts<T>(
    slice: Seq<T>,
    chunks_len: Int,
    remainder: Seq<T>,
    chunk_size: Int,
    ...
    pearlite! {
        remainder.len() == slice.len() % chunk_size
            && slice.len() == chunks_len * chunk_size + remainder.len()
    ...
    fn produces(self, visited: Seq<Self::Item>, remaining: Self) -> bool {
        pearlite! {
            0 <= visited.len() && visited.len() <= self@.0.len()
                ...
                && (forall<index> 0 <= index && index < visited.len() ==>
                    visited[index]@ == self@.0[index])
                && remaining@.1 == self@.1
                && remaining@.2 == self@.2
        }
    }
...
extern_spec! {
    impl<T> [T] {
        ...
        #[ensures(result@.0 == Seq::create(
            (self@.len() - self@.len() % chunk_size@) / chunk_size@,
            |index| self@.subsequence(
                index * chunk_size@,
                (index + 1) * chunk_size@,
            ),
        ))]
        ...
        #[ensures(chunks_exact_remainder_facts(
            self@,
            result@.0.len(),
            result@.1,
            chunk_size@,
        ))]
        #[ensures(result@.1 == self@.subsequence(
            self@.len() - result@.1.len(),
            self@.len(),
        ))]
        #[ensures(chunks_exact_partition(self@, result@.1))]
    ...
    impl<'a, T> Iterator for ChunksExact<'a, T> {
        ...
        #[ensures(match result {
            Some(chunk) => (*self)@.0.len() > 0
                && chunk@ == (*self)@.0[0]
                && (^self)@.0 == (*self)@.0.subsequence(1, (*self)@.0.len()),
            None => (*self)@.0 == Seq::empty() && (^self)@.0 == (*self)@.0,
        })]
        #[ensures(match result {
